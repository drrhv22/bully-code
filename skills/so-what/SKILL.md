...
name: so-what
description: >-
  Point so-what at any claim, paragraph, feature, pitch, headline, mission statement, or product
  line and it recursively interrogates "so what?" — drilling each answer one level deeper until it
  hits either THE POINT (the real reason anyone should care) or a DEAD END (motion without meaning).
  It is a relevance interrogation, not a flaw audit: it does not ask whether the work is good, it
  asks whether there is a "there" there. Use this skill whenever the user wants to find the real
  point, cut the fluff, test if something actually matters, pressure-test relevance, strip the
  buzzwords, or find out whether a claim leads anywhere. Trigger even when the user doesn't say "so
  what" — e.g. "what's the actual point of this", "why should anyone care", "is there a there there",
  "cut to why this matters", "strip the fluff from this", "does this actually mean anything", "get to
  the bottom line", "is this just buzzwords", "find the real takeaway", "what is this really saying".
  Do NOT use for flaw-hunting or brutal teardowns (that is bully), for line-editing/proofreading, or
  for summarizing — so-what tests whether a point EXISTS, it does not list weaknesses or condense text.
---

# So-What

So-what takes a claim, paragraph, feature, pitch, headline, or mission statement and does one thing relentlessly: it asks **"so what?"** of the claim, then asks "so what?" of the answer, then again — drilling one level deeper each time — until it strikes either **the point** (a consequence a real person would actually act on or feel) or a **dead end** (the chain runs out of meaning and just loops or restates itself). It does not assess quality. It does not list flaws. It tests one thing: whether there is a **there** there.

## The one rule that defines this skill

**Every rung must follow honestly from the text.**

The ladder is only worth anything if each rung is an *honest* answer the material actually supports — not a point you wish were there. The skill's entire power is that it refuses to let an answer count until it survives the next "so what?". That refusal is worthless the instant you smuggle in meaning the source never earned. If a pitch says "we use blockchain" and states no benefit, you do not get to invent "so it's tamper-proof, so users can trust it" — *they* didn't say that; you'd be writing their point for them and then congratulating them for it.

So the craft is: **answer each "so what?" only with what the text supports or what follows by undeniable common sense, drill until it either lands on a real consequence or stops advancing, then report which one happened — honestly.** Manufacturing a point the material doesn't support is the one absolutely forbidden move. A **dead end is not a failure of the skill — it is the most valuable result it produces.** Most fluff survives precisely because no one asked "so what?" twice. Find the rung where it stops, and name it.
...
- **No hostility, no roasting, no flaw-hunting.** You are not attacking the work or hunting weaknesses — that is bully's job. You are testing for a point. Boredom, not contempt, is the register.
...
Get the actual material first — you cannot interrogate a claim you're paraphrasing from memory. If it's a URL or a file, fetch or read it and quote the **exact** opening claim; if it's pasted text, work from the words on the page. Then:
