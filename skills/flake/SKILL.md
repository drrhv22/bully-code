...
name: flake
description: >-
  Point flake at a test suite, a test file, or pasted tests and it audits them for tests that pass
  for the wrong reason — green checkmarks that prove nothing. It does NOT find bugs and does NOT
  report coverage percentages; it judges one thing: whether each test's assertions actually constrain
  the behavior the test claims to cover. It flags hollow tests (calls the function, never checks the
  output), tests that assert on the mock instead of the code, tautologies (assert True, assert 2==2,
  asserting a literal you just set), tests of the framework instead of your logic, over-mocked tests
  where no production path runs, blessed snapshots no one verified, and assert-no-exception passing as
  behavior coverage — and proves every call with the exact line. Use this skill whenever the user
  wants a test-quality audit, a test review, or says things like "are my tests any good", "do these
  tests actually test anything", "audit my test suite", "is this test meaningful", "why is coverage
  high but bugs still ship", "are these tests real", "check my tests", "are these passing for the
  right reason", "this test is green but I don't trust it", or "find the fake tests". Trigger even
  without the word "flake". Do NOT use to hunt for bugs in the code under test (that's a code review),
  to measure or improve coverage percentage, to write net-new tests from scratch, or to debug a
  genuinely flaky/intermittent test — this skill judges whether a passing test means anything.
---

# Flake

Flake reads a test suite and judges exactly one thing: whether each passing test actually proves anything. Not whether the code under test is correct — that's a code review. Not what percentage of lines run — that's a coverage tool, and coverage is the lie this skill exists to catch. A line can be executed by a test, counted as "covered," and still be completely untested, because the test never checks what the line *did*. A test earns its green only if, when the behavior it names breaks, the test goes red. Everything else is theater.
...
- **File / repo path:** read the test files. For each test, also glance at the function under test when you can — you need to know whether the assertions actually pin its output. If mocks are involved, trace whether *any* production code path still runs, or whether the test only exercises the stubs.
- **Pasted tests:** work strictly from what's shown. If a test's honesty depends on a helper or fixture you can't see (a custom `assert_valid()`, a factory that may or may not hit real code), say the verdict is contingent and name what you'd need to confirm it. Never assume the hidden helper is hollow *or* sound.
...
Default is the full audit: every lens, every hollow test, all three sections. Tune with flags:
